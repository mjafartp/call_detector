...
class CallDetectorCallListner internal constructor(
...
//                val duration = Duration.between(time ?: ZonedDateTime.now(), ZonedDateTime.now())
                ...
                if (previousState == TelephonyManager.CALL_STATE_OFFHOOK && callType == CallType.INCOMING) {
                    // Incoming call ended
                    Log.d(CallDetectorPlugin.PLUGIN_NAME, "Phone State event IDLE (INCOMING ENDED) with number - $incomingNumber")
//                    notifyFlutterEngine(CallType.INCOMING, duration.toMillis() / 1000, incomingNumber!!)
                } else if(callType == CallType.OUTGOING) {
                    // Outgoing call ended
                    Log.d(CallDetectorPlugin.PLUGIN_NAME, "Phone State event IDLE (OUTGOING ENDED) with number - $incomingNumber")
//                    notifyFlutterEngine(CallType.OUTGOING, incomingNumber!!)
                }
            ...
            TelephonyManager.CALL_STATE_OFFHOOK -> {
                Log.d(CallDetectorPlugin.PLUGIN_NAME, "Phone State event STATE_OFFHOOK")
            ...
            TelephonyManager.CALL_STATE_RINGING -> {
                Log.d(CallDetectorPlugin.PLUGIN_NAME, "Phone State event PHONE_RINGING number: $incomingNumber")
                callType = CallType.INCOMING
                previousState = TelephonyManager.CALL_STATE_RINGING
                notifyFlutterEngine(CallType.INCOMING, incomingNumber!!)
            }
        }
    }

    private fun notifyFlutterEngine(type: CallType, number: String){
        val arguments = ArrayList<Any?>()

        // Initialize flutter engine
        if (sBackgroundFlutterEngine == null) {
            Log.d("What is context", "${context}")
            callbackHandler = context.getSharedPreferences(
                CallDetectorPlugin.PLUGIN_NAME,
                Context.MODE_PRIVATE
            ).getLong(CallDetectorPlugin.CALLBACK_SHAREDPREFERENCES_KEY, 0)
            callbackHandlerUser = context.getSharedPreferences(
                CallDetectorPlugin.PLUGIN_NAME,
                Context.MODE_PRIVATE
            ).getLong(CallDetectorPlugin.CALLBACK_USER_SHAREDPREFERENCES_KEY, 0)
            Log.d("Call back handler", "$callbackHandler $callbackHandlerUser")
            if (callbackHandler == 0L || callbackHandlerUser == 0L) {
                Log.e(CallDetectorPlugin.PLUGIN_NAME, "Fatal: No callback registered")
                return
            }
            Log.d(CallDetectorPlugin.PLUGIN_NAME, "Found callback handler $callbackHandler")
            Log.d(CallDetectorPlugin.PLUGIN_NAME, "Found user callback handler $callbackHandlerUser")
            ...
            if (callbackInfo == null) {
                Log.e(CallDetectorPlugin.PLUGIN_NAME, "Fatal: failed to find callback")
                return
            }
        ...
        channel = MethodChannel(
            ...
            CallDetectorPlugin.PLUGIN_NAME + "_background"
        )
